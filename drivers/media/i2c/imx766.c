...
#define IMX766_PIXEL_RATE_BINNING	448000000 //348800000
...
#define IMX766_REG_EXPOSURE_H 0x0202
#define IMX766_REG_EXPOSURE_L 0x0203
#define IMX766_EXPOSURE_MIN 4
...
#define IMX766_FETCH_AGAIN_H(VAL)    (((VAL) >> 8) & 0x3F)
...
// Define missing macro if not present
...
struct imx766_id_name {
	u32 id;
	char name[RKMODULE_NAME_LEN];
};

static const struct imx766_id_name imx766_module_info[] = {
	{0x36, "GuangDongLiteArray"},
	{0x0d, "CameraKing"},
	{0x00, "Unknown"}
};

static const struct imx766_id_name imx766_lens_info[] = {
	{0x47, "Sunny 3923C"},
	{0x07, "Largen 9611A6"},
	{0x00, "Unknown"}
};
...
static int imx766_write_array(struct i2c_client *client,const struct regval *regs)
{
	u32 i;
	int ret = 0;
...
int imx766_read_reg(struct i2c_client *client, u16 reg,unsigned int len, u32 *val)
{
...
int imx766_read_dw9800w_reg(struct i2c_client *client, u8 reg,unsigned int len, u32 *val)
{
	struct i2c_msg msgs[2];
	u8 *data_be_p;
	__be32 data_be = 0;
...
int imx766_write_dw9800w_reg(struct i2c_client *client, u8 reg,u32 len, u32 val)
{	
	...
	msgs[0].addr = 0x0c;
	msgs[0].flags = 0;
	msgs[0].len = 1+len;//1 bytes reg and len 
	msgs[0].buf = buf;
	...
	while (i<WRITE_COUNT)
	{
		...
		{
			pr_err("lxs %s fail write 9800w reg:%d data:%d",__func__,reg,val);
			i++;
		}
	}
	if (i>=WRITE_COUNT)
...
static int imx766_read_otp(struct i2c_client *client, u8 reg,unsigned int len, u32 *val)
{
	struct i2c_msg msgs[2];	
	u8 *data_be_p;
	__be32 data_be = 0;
	...
	if (ret != ARRAY_SIZE(msgs))
	{
		pr_err("lxs %s fail imx766 read otp:%d ",__func__,reg);
		return -EIO;
	}
...
static int __imx766_start_stream(struct imx766 *imx766)
{
	...
	if (ret)
	{
		pr_err("lxs %s error imx766_write_array(imx766->client, imx766->cur_mode->reg_list); 001",__func__);
		return ret;
    }
				...
				struct v4l2_mbus_config *config)
{
		...
		val = 1 << (IMX766_LANES - 1) |
			  V4L2_MBUS_CSI2_CHANNEL_0;
		//config->type = V4L2_MBUS_CSI2_DPHY; 
		config->type = V4L2_MBUS_CSI2_CPHY;
		config->bus.mipi_csi2.flags = 0;//FIXME: Set appropriate flags
...
static int imx766_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_EXPOSURE:
	   ...
	   ret = imx766_write_reg(imx766->client,
		...
		//printk("tianma IMX766_REG_EXPOSURE ctrl->val:%d\n",ctrl->val);
		break;
...
static int imx766_initialize_controls(struct imx766 *imx766)
{
	...
	imx766->pixel_rate = v4l2_ctrl_new_std(handler, NULL,
		V4L2_CID_PIXEL_RATE, 0, IMX766_PIXEL_RATE_FULL_SIZE,
		1, IMX766_PIXEL_RATE_FULL_SIZE);
#else
	imx766->pixel_rate = v4l2_ctrl_new_std(handler, NULL,
				   ...
				   struct i2c_client *client)
{
	...
	for (i=0; i<5; i++){
	 ...
	 pr_err("lxs %s imx766 id:0x%x 001",__func__,id);
	if (id != CHIP_ID) {
		dev_err(dev, "Unexpected sensor id(%06x), ret(%d)\n", id, ret);
		return -ENODEV;
	}
	
	ret = imx766_read_dw9800w_reg(client, DW9800W_REG_CHIP_ID, IMX766_REG_VALUE_08BIT, &id);
    pr_err("lxs %s imx766 9800w id:0x%x 001",__func__,id);
#if 0 //move VCM test	
	imx766_write_dw9800w_reg(client, 0x02, 1, 0x00); 
	imx766_write_dw9800w_reg(client, 0x03, 2, 0x00);
	usleep_range(5000, 10000);
	pr_err("lxs %s imx766 9800w pos:1000 001",__func__);
	imx766_write_dw9800w_reg(client, 0x03, 2, 1000);
	usleep_range(500000, 600000);
	pr_err("lxs %s imx766 9800w pos:0 001",__func__);
	imx766_write_dw9800w_reg(client, 0x03, 2, 0x00);
#endif
    //otp 	
	id=0;
	ret = imx766_read_otp(client, 0x00, 4, &id);
	pr_err("lxs %s imx766 read otp pos:0 0x%x 010",__func__,id);
	
	id=0;
	ret = imx766_read_otp(client, 0x04, 4, &id);
	pr_err("lxs %s imx766 read otp pos:4 0x%x 011",__func__,id);
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (IS_ERR(imx766->power_gpio))
	{	
        dev_warn(dev, "lxs Failed to get power-gpios\n");
    }else
	{
	}
